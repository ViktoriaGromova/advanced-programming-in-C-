...
class String{
    // Сначала идет инициализация в том порядке, от которых они зависит по инициализации
    // Поля инициализируются в том порядке, в каком они объявлены в классе, и 
    // независимо от того, как вы передаете в списке конструктора
    size_t size = 0;
    char* str = nullptr;
...
public:
    ...
    // Делегирующие конструкторы нельзя использовать со списком инициализации полей 
    ...
    // Конструктор, работающий с использованием Initializer_list
    String(const std::initializer_list<char>& lst): size(lst.size()), str(new char[size]){
        std::copy(lst.begin(), lst.end(), str);
    }
...
int main(){
    ...
    // Как не спутать конструктор копирования с опреатором присваивания
    // Оператор присваивания - это когда используем уже существующий объект, и хотим его переопределить
